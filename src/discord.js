...
function buildTicketEmbed(ticket, discord) {
  ...
  if (ticketUrl) {
    embed.addFields({
      name: `${emoji("horizon_tool", e.tool)} Open Ticket`,
      value: `[Open ticket](${ticketUrl})`
    });
  }

  if (inviteUrl) {
    embed.addFields({ name: "HorizonDev", value: `[Open Discord server](${inviteUrl})` });
  }
    ...
    components.push(
      new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setURL(ticketUrl)
          .setEmoji(discord.emoji?.tool ? { id: discord.emoji.tool, name: "horizon_tool" } : "🔗")
      )
    );
  }
